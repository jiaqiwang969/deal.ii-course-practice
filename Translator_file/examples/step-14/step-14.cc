...
namespace Step14
{
  ...
  // 正如介绍中提到的，该程序的重要部分只是从 step-13
  // 的例子程序中拿过来的。因此，我们只对那些新的东西进行评论。
  ...
  namespace Evaluation
  {
    ...
    // 鉴于 <code>PointValueEvaluation</code>
    // 类的接口，这个类的声明没有提供什么惊喜，构造函数也没有。
          ...
          if (cell->vertex(vertex) == evaluation_point)
            {
              ...
              // 相反，我们必须在这个单元上，在某一点上评估有限元场。如你所知，在某一点上评估有限元场是通过
              // <code>FEValues</code> 类完成的，所以我们使用它。问题是：
              // <code>FEValues</code>
              // 对象需要给定一个正交公式，然后可以计算正交点的有限元量值。在这里，我们并不想做正交，我们只是想指定一些点!
              // 尽管如此，还是选择同样的方式：使用一个特殊的正交规则，点在顶点，因为这些是我们感兴趣的。适当的规则是梯形规则，所以这就是我们上面使用该规则的原因。
              // 因此：在这个单元格上初始化 <code>FEValues</code> 对象。
    ...
    // 然而，我们也想看看生成的网格。这也可以通过一个这样的类来完成。它的结构类似于前面例子程序中的
    // <code>SolutionOutput</code>
    // 类，所以我们在这里不做更详细的讨论。此外，这里所使用的一切都已经在前面的例子程序中使用过了。
  ...
  namespace LaplaceSolver
  {
    ...
    // 这个类几乎没有变化，只是多声明了两个函数。  <code>output_solution</code>
    // 将用于从派生类计算的实际解决方案中生成输出文件，以及
    // <code>set_refinement_cycle</code>
    // 函数，测试框架通过该函数将细化周期的编号设置为该类中的一个局部变量；该编号随后将用于生成解决方案输出的文件名。
    ...
    private:
      ...
      // 该类的其余部分基本上也是 step-13
      // 的副本，包括使用WorkStream框架并行计算线性系统所需的数据结构和函数。
    ...
    // 现在是实现线性系统类中动作的函数。首先，构造函数将所有数据元素初始化为正确的大小，并设置了一些额外的数据结构，例如由于悬挂节点而产生的约束。由于设置悬空节点和找出矩阵的非零元素是独立的，所以我们以并行方式进行（如果库被配置为使用并发，至少是这样；否则，这些动作是按顺序执行的）。注意，我们只启动一个线程，并在主线程中做第二个动作。由于只生成一个线程，我们在这里不使用
    // <code>Threads::TaskGroup</code>
    // 类，而是直接使用创建的一个任务对象来等待这个特定任务的退出。这个方法与我们在上面
    // <code>Solver::assemble_linear_system()</code> 中使用的方法大致相同。

    // 注意，获取 <code>DoFTools::make_hanging_node_constraints</code>
    // 函数的地址有点麻烦，因为实际上有三个这个名字的函数，每个支持的空间维度都有一个。在C++中，获取重载函数的地址有些复杂，因为在这种情况下，操作符
    // <code>&</code>
    // 会返回一组值（所有具有该名称的函数的地址），然后选择正确的函数是下一步的事情。如果上下文决定采取哪一个（例如通过分配给一个已知类型的函数指针），那么编译器可以自己做，但是如果这组指针应作为一个采取模板的函数的参数，编译器可以选择所有的，而不偏向于一个。因此，我们必须向编译器说明我们想要哪一个；为此，我们可以使用cast，但为了更清楚，我们把它分配给一个具有正确类型的临时
    ...
    //  <code>PrimalSolver</code> 类除了实现 <code>output_solution</code>
    //  函数外，也基本没有变化。我们在这个程序中保留了
    //  <code>GlobalRefinement</code> and <code>RefinementKelly</code>
    //  类，它们就可以依赖这个函数的默认实现，这个函数只是输出原始解。实现双重加权误差估计的类将自行重载这个函数，以同时输出双重解。
    ...
    void RefinementWeightedKelly<dim>::refine_grid()
    {
      ...
      // 接下来用给与构造函数的值来衡量指标向量中的每个条目，在单元格中心进行评估。我们需要将结果写入对应于当前单元的向量条目中，我们可以通过使用
      // CellAccessor::active_cell_index().
      // 询问该单元在所有活动单元中的索引来获得（实际上，对于我们在循环中处理的第一个单元，该索引为0，第二个单元为1，等等，我们也可以使用一个整数计数器来跟踪该索引；但是使用
      // CellAccessor::active_cell_index() 使其更加明确。
  ...
  // 基本上，这个想法是这样的：让我们为每一组数据都有一个结构，在这个结构中，我们把描述一个测试案例的所有东西都打包：这里，这些是两个子类，一个叫
  // <code>BoundaryValues</code> ，用于精确解的边界值，一个叫
  // <code>RightHandSide</code>
  // ，然后是生成粗略网格的方法。由于前面的例子程序的解看起来像弯曲的山脊，所以我们在这里用这个名字来表示包围的类。请注意，两个内层类的名称对于所有包围的测试案例类必须是相同的，同时我们将维度模板参数附加到包围类而不是内层类，以使进一步的处理更简单。
  // 从语言的角度来看，用命名空间来封装这些内部类会比用结构来封装更好。然而，命名空间不能作为模板参数给出，所以我们使用一个结构来允许第二个对象从其给定的参数中选择。当然，这个封闭的结构除了它所声明的类之外，没有任何成员变量，还有一个静态函数来生成粗略的网格；一般来说，它永远不会被实例化）。)

  //然后
  //想法是如下的（这是正确的时间，也可以简单看看下面的代码）：我们可以为边界值和右手边生成对象，只需将外层类的名字作为模板参数给一个类，我们在这里称之为
  //<code>Data::SetUp</code>
  //，然后它为内部类创建对象。在这种情况下，为了得到所有描述弧形山脊解决方案的特征，我们将简单地生成一个
  //<code>Data::SetUp@<Data::CurvedRidge@></code>
  //的实例，而我们需要知道的关于该解决方案的一切都将是该对象的静态成员变量和函数。

  // 在这种情况下，这种方法可能显得有些多余，但是一旦某种设定不仅有迪里希特边界值和右手函数的特征，而且还有材料属性、诺伊曼值、不同的边界描述符等，就会变得非常方便。在这种情况下，
  // <code>SetUp</code> 类可能由十几个对象组成，而每个描述符类（如下面的
  // <code>CurvedRidges</code>
  // 类）都必须提供这些对象。然后，你会很高兴，只需在一个地方改变
  // <code>SetUp</code>
  // 类的模板参数，而不是在很多地方改变，就能从一组数据改变到另一组。

  // 有了这个不同测试用例的框架，我们就快完成了，但还有一件事：现在我们可以通过改变一个模板参数，静态地选择要选择的数据集。为了能够动态地做到这一点，即在运行时，我们需要一个基类。我们以明显的方式提供这个基类，见下文，用虚拟抽象函数。这迫使我们引入第二个模板参数
  // <code>dim</code>
  // ，我们需要这个基类（这可以通过一些模板魔法来避免，但我们省略），但这就是全部。

  // 添加新的测试用例现在很简单，你不需要接触框架类，只需要一个类似于
  // <code>CurvedRidges</code>  的结构。

  namespace Data
  {
    // @sect4{The SetUpBase and SetUp classes}

    // 基于上述描述， <code>SetUpBase</code>
    // 类就看起来如下。为了允许用这个类来使用 <code>SmartPointer</code>
    // 类，我们从 <code>Subscriptor</code> 类派生出来。
    ...
    // 用于描述 <code>curved ridge</code> 问题的边界值和右手边的类已经在 step-13
    // 示例程序中使用，那么就像这样。
    ...
    // 如上所述，本例的网格是正方形[-1,1]^2，其中的正方形[-1/2,1/2]^2为孔。我们将粗略的网格创建为4乘以4的单元，中间的四个单元缺失。要了解网格的具体样子，最简单的方法可能是先看一下本教程程序的
    // "结果
    // "部分。一般来说，如果你想了解更多关于创建网格的信息，无论是像我们在这里所做的那样从头开始，还是使用其他技术，你都应该看一下
    // step-49  。
  ...
  // 你现在已经读完了这个框架，你可能会想，为什么我们没有选择直接把实现某种设置的类（比如
  // <code>CurvedRidges</code> 类）作为派生自 <code>Data::SetUpBase</code>
  // 的类来实现。事实上，我们可以很好地这样做。唯一的原因是，这样我们就必须在
  // <code>CurvedRidges</code>
  // 类中为解决方案和右手边的类设置成员变量，以及重载基类的抽象函数来访问这些成员变量的成员函数。
  // <code>SetUp</code>
  // 类的唯一原因是让我们不必再重申这些成员变量和函数，而这些成员变量和函数在所有这些类中都是必要的。在某种程度上，这里的模板机制只是提供了一种方法，为一些依赖于外部量的函数提供默认的实现，因此不能使用正常的虚拟函数来提供，至少在没有模板的帮助下不能。

  // 然而，可能有很好的理由来实际实现从 <code>Data::SetUpBase</code>
  // 派生的类，例如，如果解或右手边的类需要带参数的构造函数，而
  // <code>Data::SetUpBase</code>
  // 类无法提供。在这种情况下，子类化是一个值得考虑的策略。对于特殊情况的其他可能性是，从
  // <code>Data::SetUp@<SomeSetUp@></code> 派生，其中 <code>SomeSetUp</code>
  // 表示一个类，或者甚至明确地专门化 <code>Data::SetUp@<SomeSetUp@></code>
  // 。后者允许透明地使用 <code>SetUp</code>
  // 类用于其他设置的方式，但对特殊参数采取特殊行动。

  // 赞成这里采取的方法的最后一个意见是：我们无数次发现，当开始一个项目时，参数的数量（通常是边界值，右侧，粗略的网格，就像这里）很小，测试案例的数量也很小。然后，人们一开始就把它们手工编码成一些
  // <code>switch</code>
  // 的语句。随着时间的推移，项目的增长，测试用例的数量也在增长。
  // <code>switch</code>
  // 语句的数量也随之增长，它们的长度也是如此，人们开始想办法考虑不可能的例子，其中域、边界值和右手边不再适合在一起，并且开始失去对整个结构的概述。事实证明，把属于某个测试用例的所有东西都封装到一个自己的结构中是值得的，因为它把属于一个测试用例的所有东西都放在一个地方。此外，它允许把这些东西都放在一个或多个文件中，这些文件只用于测试用例和它们的数据，而不需要把它们的实际实现与程序的其他部分联系起来。
  ...
  namespace DualFunctional
  {
    ...
    // 我们可以像实现各自的评价对象那样，在这个评价点上取每个形状函数phi_i的梯度的平均值。然而，我们采取了一个略微不同的方法：我们简单地取该点周围所有单元格的平均值。哪些单元
    // <code>surrounds</code>
    // 是评估点，这个问题取决于网格宽度，包括那些单元的中点到评估点的距离小于单元的直径的单元。
      ...
      Vector<double> &       rhs) const
    {
      ...
      for (const auto &cell : dof_handler.active_cell_iterators())
        if (cell->center().distance(evaluation_point) <= cell->diameter())
          {
            // 如果我们找到了这样的单元，那么就初始化 <code>FEValues</code>
            // 对象，并整合每个形状函数梯度的x分量，以及总面积/体积的单位函数。
  ...
  namespace LaplaceSolver
  {
    ...
    // 与上面的  <code>PrimalSolver</code>  类相同，我们现在实现一个
    // <code>DualSolver</code>
    // 。它具有所有相同的特征，唯一的区别是它不接受一个表示右侧对象的函数对象，而现在接受一个
    // <code>DualFunctionalBase</code>
    // 对象，它将集合对偶问题的右侧向量。这个类的其余部分是相当琐碎的。

    // 由于原始求解器和对偶求解器将使用相同的三角形，但是不同的离散化，现在很清楚为什么我们将
    // <code>Base</code> 类变成了虚拟类：因为最终类将从
    // <code>PrimalSolver</code> 以及 <code>DualSolver</code>, it would have two
    // <code>Base</code>
    // 实例中派生，我们是不是应该将继承标记为虚拟。因为在许多应用中，基类会存储更多的信息，而不仅仅是需要在原始求解器和对偶求解器之间共享的三角形，所以我们通常不希望使用两个这样的基类。
    ...
    private:
      // 在私有部分，我们有两个函数，用来调用原始类和双基类的
      // <code>solve_problem</code> 函数。这两个函数将被本类的
      // <code>solve_problem</code> 函数所平行调用。
      ...
      // 在计算单元和面的误差估计时，我们需要一些辅助对象，例如
      // <code>FEValues</code> 和 <code>FEFaceValues</code>
      // 函数，但也需要一些临时对象来存储原始和对偶解的值和梯度，例如。这些字段在三个函数中都是需要的，这些函数分别在单元格、规则面和不规则面上做积分。
      ...
      // WorkStream::run
      // 一般要有一个从头开始的对象和一个拷贝对象。在这里，由于与我们在 step-9
      // 中讨论梯度的近似计算时类似的原因，我们实际上不需要一个 "拷贝数据
      // "结构。既然WorkStream坚持要有一个这样的结构，我们就声明一个空的结构，除了存在之外什么都不做。

      struct WeightedResidualCopyData
      {};

      // 关于误差估计器的评估，我们有一个驱动函数，使用  WorkStream::run()
      // 在每个单元上调用第二个函数。
    ...
    // 在这个类的实现中，我们首先有 <code>CellData</code> and
    // <code>FaceData</code> 成员类的构造函数，以及
    // <code>WeightedResidual</code>
    // 构造函数。它们只将字段初始化为正确的长度，所以我们不必过多地讨论它们。
    ...
    // 现在，变得更加有趣了： <code>refine_grid()</code>
    // 函数要求误差估计器计算单元格的误差指标，然后使用其绝对值进行网格细化。
    ...
    // 由于我们想同时输出原始解和对偶解，我们重载了 <code>output_solution</code>
    // 函数。这个函数唯一有趣的特点是，原始解和对偶解是在不同的有限元空间上定义的，这不是
    // <code>DataOut</code>
    // 类所期望的格式。因此，我们必须将它们转移到一个共同的有限元空间。由于我们只想从质量上看到这些解，所以我们要争夺将对偶解内插到（较小的）原始空间。对于插值，有一个库函数，它接收一个AffineConstraints对象，包括悬挂节点约束。其余的都是标准的。
    ...
    void WeightedResidual<dim>::output_solution() const
    {
      ...
      // 添加我们想要输出的数据向量。两个都加上， <code>DataOut</code>
      // 函数可以处理你想写到输出的多少个数据向量。
    ...
    WeightedResidual<dim>::estimate_error(Vector<float> &error_indicators) const
    {
      ...
      // 首先，让我们对原始解进行插值：它被逐格插值到我们已经解决了对偶问题的有限元空间中：但是，还是和
      // <code>WeightedResidual::output_solution</code>
      // 函数一样，我们首先需要创建一个包括悬挂节点约束的AffineConstraints对象，但这次是对偶有限元空间的。
      ...
      FETools::interpolate(PrimalSolver<dim>::dof_handler,
                           ...
                           primal_solution);
      //然后
      //为了计算数值逼近的对偶解z插值到原始解的有限元空间并从z中减去：使用
      //<code>interpolate_difference</code>
      //函数，在对偶解的元素空间中得到(z-I_hz)。
      ...
      // 我们已经用一个-1e20的值初始化了这个地图，因为如果出现问题，我们因为某些原因无法计算某个面的值，这个值就会在结果中显示出来。其次，这个初始化已经让
      // std::map
      // 对象分配了它可能需要的所有对象。这一点很重要，因为我们将从并行线程写进这个结构，如果地图需要分配内存，从而重塑其数据结构，那么这样做就不是线程安全的。换句话说，初始化使我们不必在线程每次写入（和修改）该地图的结构时通过互斥来同步。
    ...
    {
      // 由于WorkStream的原因，
      // estimate_on_one_cell需要一个CopyData对象，即使它没有被使用。下一行将对这个未使用的变量发出警告。

      (void)copy_data;

      // 每个单元的第一个任务是计算这个单元的剩余贡献，并把它们放入
      // <code>error_indicators</code>  变量中。
      ...
      for (const auto face_no : GeometryInfo<dim>::face_indices())
        {
    ...
    // 另一方面，误差估计的边缘项的计算并不那么简单。首先，我们必须区分有悬挂节点和无悬挂节点的面。因为这是一种简单的情况，我们首先考虑一个面上没有悬挂节点的情况（我们称之为
    // "常规 "情况）。
      ...
      FaceIntegrals &             face_integrals) const
    {
      ...
      // 第一步是获取本单元上有限元场的正交点的梯度值。为此，初始化
      // <code>FEFaceValues</code>
      // 对象，对应于面的这一侧，并使用该对象提取梯度。
      ...
      // 如果我们有了这个，那么我们需要找出相邻单元格的哪个面，也就是说，
      // <code>how-many'th</code>
      // 这个单元格是这个面后面的单元格的相邻面。为此，有一个函数，我们将结果放入一个变量，名称为
      // <code>neighbor_neighbor</code>  。

      const unsigned int neighbor_neighbor =
        cell->neighbor_of_neighbor(face_no);

      // 然后定义一个邻近单元的缩写，在该单元上初始化 <code>FEFaceValues</code>
      // 对象，并提取该单元上的梯度。
      ...
      FaceIntegrals &             face_integrals) const
    {
      ...
      for (unsigned int subface_no = 0; subface_no < face->n_children();
           ++subface_no)
        {
          // 再从一些检查开始：得到一个指向当前子面后面的单元格的迭代器，并检查其面是否是我们正在考虑的子面。如果不是这样，那么要么是上面调用的
          // <code>neighbor_neighbor</code>
          // 函数存在错误，要么--更糟糕的是--库中的某些函数没有遵守关于单元格、它们的子面的一些基本假设。在任何情况下，即使这个断言不应该被触发，谨慎一点也无妨，而且在优化模式的计算中，这个断言无论如何都会被删除。
  ...
  public:
    ...
    struct ProblemDescription
    {
      ...
      // 由于我们允许使用不同的细化标准（全局细化、通过凯利误差指标细化，可能有一个权重，以及使用双重估计器），定义一些枚举值，并随后定义一个该类型的变量。它将默认为
      ...
      // 接下来是一个函数，作为 <code>RefinementWeightedKelly</code>
      // 类的权重。这个指针的默认值是零，但是如果你想使用
  ...
  try
    {
      ...
      // 在这里，我们也可以使用  <code>global_refinement</code>  或
      // <code>weighted_kelly_indicator</code>
      // 。请注意，所给出的关于对偶有限元、对偶函数等信息只对给定的细化准则选择很重要，否则就会被忽略。
      ...
      // 然后设置测试案例的描述，即域、边界值和右手边。这些都是预先打包在类中的。我们在这里采用
      // <code>Exercise_2_3</code>  的描述，但你也可以使用
      // <code>CurvedRidges@<dim@></code>  。

      descriptor.data =
        std::make_unique<Data::SetUp<Data::Exercise_2_3<dim>, dim>>();

      // 接下来先设置一个二元函数，然后设置一个评价对象的列表。我们默认选择在一个评价点对值进行评价，由评价和二元函数类命名空间中的
      // <code>PointValueEvaluation</code> 类代表。你也可以设置
      // <code>PointXDerivativeEvaluation</code>
      // 类来代替评价点上的值的x-derivative。

      // 注意，双功能和评价对象应该匹配。然而，你可以给你想要的评价函数，所以你可以在每一步之后让点值和导数都得到评价。
      // 一个这样的附加评价是在每一步中输出网格。
