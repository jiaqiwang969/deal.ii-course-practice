...
def format_block(lines, infostr=""):
    ...
    if not lines[0].strip().startswith("/**"):
        print("%s error, ignoring code block with junk in same line before" %
              infostr, file=sys.stderr)
        return lines
    if not lines[-1].strip().endswith("*/"):
        print("%s error, ignoring code block not ending at end of line" %
              infostr, file=sys.stderr)
        return lines
    ...
    ops_startline = ["@subsection", "@section", "@defgroup", "<li>", "@param", "@returns", "@warning", "@ingroup", "@author", "@date", "@related", "@relates", "@relatesalso", "@deprecated", "@image", "@return",
                     "@brief", "@attention", "@copydoc", "@addtogroup", "@todo", "@tparam", "@see", "@note", "@skip", "@skipline", "@until", "@line", "@dontinclude", "@include"]

    # subset of ops_startline that does not want stuff from the next line appended
    # to this.
    ops_also_end_paragraph = [
        "@image", "@skip", "@skipline", "@until", "@line", "@dontinclude", "@include"]
        ...
        if one_in(ops_separate_line, lines[idx]):
            ...
            for it in ops_separate_line:
                if it in thisline and thisline != it:
                    print("%s warning %s not in separate line" %
                          (infostr, it), file=sys.stderr)
            ...
            if not thisline.split(" ")[0] in ops_title_line:
                print("%s warning: @page/@name not at start of line" %
                      (infostr), file=sys.stderr)
            ...
            if not thisline.startswith("@ref") and not thisline.startswith("(@ref"):
                print("%s warning %s not at start of line" %
                      (infostr, "@ref"), file=sys.stderr)
            ...
            if len(withquotes) == 3:
                ...
                if len(withquotes[0].strip().split(' ')) != 2:
                    print("%s warning @ref line looks broken" %
                          (infostr), file=sys.stderr)
                ...
                print("%s warning @ref quotes are not in single line" %
                      (infostr), file=sys.stderr)
                remain = ''
            ...
            while True:
                ...
                if thisline.strip() == "*":
                    ...
                    if thisline[0:len(start)].strip() == "":
                        ...
                        print("%s Error: wrong formatting inside @code block" %
                              infostr, file=sys.stderr)
                        thisline = start + thisline.strip()
                else:
