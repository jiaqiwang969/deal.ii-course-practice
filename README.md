...
7. 确保你创建了一个 "线性弹性问题测试器"类来测试你的问题。使用gtest测试你的问题，并创建纯剪切和纯压缩/扩张测试案例。
...
3. 创建一个 "斯托克斯 "问题，派生自 "基块问题 "类，并组装混合斯托克斯系统。组装混合斯托克斯系统。

4. 使用 "线性运算器 "类设施为斯托克斯系统创建一个有效的Schur 补数求解器（按照 "第60步 "的讨论进行块系统的解析）。块系统的解决）。)

5. 从一个任意矢量值的光滑函数开始，创建一个非三段式的人造解。5. 从一个任意矢量值的平滑函数开始，创建一个非显著的人造解，并将其卷曲（这自动是无发散的）作为预期速度解。无发散）作为一个预期的速度解。误差收敛的检查 Taylor-Hood FES系统。对于以下有限元的选择会发生什么？空间，在一个均匀细化的方形网格上会发生什么？
   1. FE_Q(1)^d-FE_DGQ(0) 
   2. FE_Q(2)^d-FE_DGP(1) 
   3. FE_Q(2)^d-FE_Q(1)